	...
	internal class WkMethodManager
	{
		...
		public WkMethodManager()
		{
			...
			foreach (var m in mlist)
			{
				...
				{
					WkLogger.LogError(
						$"Method with id <color=red>{m.GetCustomAttribute<WhichKeyMethod>().UID}</color> has been registered!");
					continue;
				}
				...
				if (m.IsStatic && m.GetParameters().Length == 0)
					...
					WkLogger.LogError(
						$"Method <color=red>{m.Name}</color> can't be invoke by WhichKey!make sure it is static and has no arguments.");
			}
		}
