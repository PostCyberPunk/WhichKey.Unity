...
namespace PCP.Tools.WhichKey
{
	internal class TreeBuilder
	{
		...
		private void AddKeySetToTree(KeySet keyset)
		{
			...
			for (int i = 0; i < keyset.Keys.KeySeq.Length; i++)
			{
				int key = keyset.Keys.KeySeq[i];
				KeyNode kn = mCurrentNode.GetChildByKey(key);
				if (i == keyset.Keys.KeySeq.Length - 1)
				{
					if (kn == null)
					{
						if (keyset.CmdType == 0)
						...
						if (cmd == null)
						{
							WhichKeyManager.LogError($"<color=yellow>Hint: {keyset.Hint}</color>||<color=green>Key:{keyset.Keys.KeyLabel}</color>||<color=red>Command:{keyset.CmdType}</color> has no valid command type,skipped ");
							return;
						}
					...
					else
					{
						if (kn.isLayer && string.IsNullOrEmpty(kn.Hint) && keyset.IsLayer)
						{
							...
							WhichKeyManager.LogError($"<color=yellow>Hint: {keyset.Hint}</color>||<color=green>Key:{keyset.Keys.KeyLabel}</color> already registered To \"<color=yellow>{kn.Hint}</color>\" ,skipped ");
					}
					return;
				}
				if (kn == null)
